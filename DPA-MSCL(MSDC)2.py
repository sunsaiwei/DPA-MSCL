...
from utils.load_data_MSDC import get_blind_HP_multiscale
from utils.utils import get_cls_num_list, write_file, get_confusion_matrix, save_metrics_plot, warmup_lr_scheduler
from model.DPA_MSCL_MSDC import BCLModel,Classifier
from losses.BCLLoss import BalSCL
from losses.logitadjustLoss import LogitAdjust
from train.DPA_MSCL_train_MSDC import BCL_train, train_model, evaluate, save_predictions
...
def main():
    ...
    if args.dataset == "blind_HP":
        x_train, x_test, data_train_loader, y_train, y_test, test_loader = get_blind_HP_multiscale(
            args.data_path1, args.blind_well1, args.blind_well2, args)
    ...
    plt.title(f'Confusion Matrix for DPA-BCL(MvDC) on {args.dataset}')
    ...
    plt.title(f'Accuracy Curves for DPA-BCL(MvDC) on {args.dataset}')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    accuracy_curve_save_path = save_path + f'accuracy_curve/'
    plt.savefig(accuracy_curve_save_path + f'DPA-BCL(MvDC).png')
    plt.show()
